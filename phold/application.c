...
extern void ProcessEvent(lp_id_t me, simtime_t now, unsigned event_type, const unsigned *event_content, unsigned int event_size, void *state);
...
void ProcessEvent(lp_id_t me, simtime_t now, unsigned event_type, const unsigned *event_content, unsigned int event_size, void *state) {
	...
	switch (event_type) {
		...
		case LOOP:
			...
			if(state_ptr->buffer_count && Random() < send_probability) {
				...
				ScheduleNewEvent(NUM_LPS * Random(), timestamp, RECEIVE, to_send->data, to_send->count * sizeof(unsigned));
...
void RestoreApproximated(lp_id_t id, void *ptr) {
	...
	while(i--) {
		...
		for(unsigned j = 0; j < (unsigned)(RESTORE_PERCENTAGE * tmp->count); j++) {
