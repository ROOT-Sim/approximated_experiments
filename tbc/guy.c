...
// these flags are used in the infection structure, read below for more infos
enum _infl_t { infl_middle_age, infl_elder_age, infl_foreigner, infl_female, infl_smear, infl_count };
...
static void infect(unsigned me, struct guy_t *guy, simtime_t now)
{
	...
	lp_id_t target;
	lp_id_t i = CountDirections(me, topology);
	lp_id_t j = 0;
	while(i) {
...
static bool guy_treated_update(struct guy_t *guy, region_t *region)
{
	// we check whether this is a treated guy, else we exit
	if(guy->state != TREATED)
		return false;

	if(region->now >= t_to_healthy + guy->treatment_day) {
...
void guy_recv(unsigned me, const struct guy_t *agents, unsigned msg_size, region_t *region)
{
	unsigned agents_count = msg_size / sizeof(struct guy_t);

	while(agents_count--) {
		struct guy_t *curr_guy = rs_malloc(sizeof(struct guy_t));
...
{
	lp_id_t neighbours = CountDirections(me, topology);
	...
	while(neighbours--) {
		lp_id_t dest;
		do {
			dest = GetReceiver(me, topology, direction++);
		} while(dest == INVALID_DIRECTION);
...
bool guy_on_leave(struct guy_t *guy, region_t *region)
{
	...
	if(guy->state != SICK) {
		// non-sick guy case
		guy_dies = randomNumber < die_probability(region->now - guy->birth_day);
	} else {
		// sick guy case
		guy_dies = randomNumber < p_die_sick;
	}

	return guy_dies;
}

// this function does some shenanigans on some parameters array
// (notice that I changed the parameters arrays layout to a more consistent one)
static int infected_age(const infection_t *inf)
{
...
void guy_on_infection(const infection_t *inf, region_t *region)
{
	...
	while(infections--) {
		...
		// set infection day
		guy->infection_day = region->now;
		// set age
		guy->birth_day = region->now - infected_age(inf);
		// set origin and gender
		aux = infected_gender_origin(inf, region);
