...
{
	init_t total = {.agents_count = {num_healthy / scale_factor, num_infected / scale_factor,
			    num_sick / scale_factor, num_treatment / scale_factor, num_treated / scale_factor}};
	init_t to_send = {0};

	const lp_id_t regions = CountRegions(topology);
	lp_id_t remaining = regions, i;
